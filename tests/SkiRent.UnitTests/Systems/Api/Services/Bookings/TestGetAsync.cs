...
public class TestGetAsync
{
    ...
    public async Task WhenBookingNotFound_ReturnsFailedResult()
    {
        ...
        // Act
        var result = await _bookingService.GetAsync(bookingId, userId, null!);
    ...
    [Test]
    public async Task WhenUserIsNotOwnerAndNotAdmin_ReturnsAccessDeniedError()
    {
        // Arrange
        var bookingId = _fixture.Create<int>();
        var userId = _fixture.Create<int>();
        ...
        var booking = _fixture.Build<Booking>()
            .With(booking => booking.UserId, userId + 1)
            .Create();
        ...
        // Act
        var result = await _bookingService.GetAsync(bookingId, userId, isInRole);

        // Assert
        Assert.That(result.IsFailed, Is.True);
        Assert.That(result.Errors[0], Is.InstanceOf<BookingAccessDeniedError>());
        Assert.That(result.Errors[0].Metadata.GetValueOrDefault("bookingId"), Is.EqualTo(bookingId));
    }
}
